...
# File copied and modified from https://github.com/bytedance/ibot
...
from mim_utils.loader import ImageFolderMask
from mim_utils.evaluation.unsup_cls import eval_pred


def get_args_parser():
    parser = argparse.ArgumentParser('mim_pretraining', add_help=False)
    ...
    parser.add_argument('--dataset', default='meta_dataset', type=str,
                        choices=['meta_dataset', 'imagenet'],
    ...
    parser.add_argument('--label_path', default=None, type=str,
                        help='Please specify path to the folder containing the training labels for mim. Instructions '
                             'to download the labels are provided in the README.md')
...
def train_mim(args):
    ...
    # if the network is of hierarchical nature (i.e. swin_tiny, swin_small, swin_base)
    ...
    if args.load_from:
        utils.restart_from_checkpoint(
            os.path.join(args.load_from),  # os.path.join(args.output_dir, args.load_from),
    ...
    print(f"Starting mim training from epoch {start_epoch}!")
    for epoch in range(start_epoch, args.epochs):
        data_loader.sampler.set_epoch(epoch)
        data_loader.dataset.set_epoch(epoch)

        # ============ training one epoch of mim ... ============
