...
class ProxyAnchorLoss_Custom(WeightRegularizerMixin, BaseMetricLossFunction):
    def __init__(self, num_classes, embedding_size, prototypes=None, margin=0.1, alpha=32, **kwargs):
        super().__init__(**kwargs)

        if prototypes is not None:
            self.proxies = torch.nn.Parameter(prototypes)
        else:
