...
class MetaDatasetReader(object):
    ...
    def build_class_to_identity(self, mode):
        split = SPLIT_NAME_TO_SPLIT[mode]
        all_split_specs = self.specs_dict[SPLIT_NAME_TO_SPLIT[mode]]
...
class MetaDatasetEpisodeReader(MetaDatasetReader):
    ...
    """
    def __init__(self, mode, train_set=None, validation_set=None, test_set=None, test_type='standard', image_size=84):
    ...
    def _get_task(self, next_task, session):
        episode = self.session.run(next_task)[0]
        task_dict = {
...
class MetaDatasetBatchReader(MetaDatasetReader):
    ...
    Class that wraps the Meta-Dataset episode readers.
    """
    def __init__(self, mode, train_set, validation_set, test_set, batch_size, image_size=84):
        ...
        if mode == 'val':
            self.val_dataset_next_task = self._init_multi_source_dataset(
                validation_set, SPLIT_NAME_TO_SPLIT['val'])
        ...
        self.build_class_to_identity(mode)
    ...
    def _init_multi_source_dataset(self, items, split):
        ...
        multi_source_pipeline = pipeline.make_multisource_batch_pipeline(
            ...
            split=split, image_size=self.image_size, add_dataset_offset=True, shuffle_buffer_size=1000)
    ...
    def _init_single_source_dataset(self, dataset_name, split):
        ...
        multi_source_pipeline = pipeline.make_one_source_batch_pipeline(
            ...
            split=split, image_size=self.image_size)

        iterator = multi_source_pipeline.make_one_shot_iterator()
        return iterator.get_next()

    def _get_batch(self, next_task, session, labels_encoded):
        episode = self.session.run(next_task)[0]
        ...
        if labels_encoded is not None:
            lab_encoded = [labels_encoded.get(item, item) for item in labels]
            lab_encoded = np.asarray(lab_encoded)
            task_dict['encoded_labels'] = lab_encoded

        return self._to_torch(task_dict)

    def get_train_batch(self, session, labels_encoded=None):
        return self._get_batch(self.train_dataset_next_task, session, labels_encoded)

    def get_validation_batch(self, item, session, labels_encoded=None):
        return self._get_batch(self.validation_set_dict[item], session, labels_encoded)

    def get_test_batch(self, item, session, labels_encoded=None):
        return self._get_batch(self.test_set_dict[item], session, labels_encoded)


class MetaDataset(Dataset):

    def __init__(self, reader, length, train=True):
        self.reader = reader
        self.train = train
        self.length = length
    ...
    def __getitem__(self, idx):
        if self.train:
            sample = self.reader.get_train_batch()
        else:
            sample = self.reader.get_validation_batch()
